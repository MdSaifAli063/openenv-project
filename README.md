...
### Action
**OpenenvProjectAction**: Contains a single field
- `message` (str) - The message to echo back

### Observation
**OpenenvProjectObservation**: Contains the echo response and metadata
...
with OpenenvProjectEnv(base_url="http://localhost:8000") as env:
    ...
    for msg in ["Hello", "World", "!"]:
        result = env.step(OpenenvProjectAction(message=msg))
        print(f"Echoed: {result.observation.echoed_message}")
```
...
app = create_app(
    OpenenvProjectEnvironment,  # Pass class, not instance
    OpenenvProjectAction,
    OpenenvProjectObservation,
...
def run_episode(client_id: int):
    with OpenenvProjectEnv(base_url="http://localhost:8000") as env:
        result = env.reset()
        for i in range(10):
            result = env.step(OpenenvProjectAction(message=f"Client {client_id}, step {i}"))
...
├── client.py              # OpenenvProjectEnv client
